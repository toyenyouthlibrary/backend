    ...
    function __construct($string){
        ...
        if($this->error == ""){
            ...
            if (isset($string) && $string != "") {
                ...
                $sql = "SELECT bookID, title, author, language, shelfID FROM lib_Book WHERE LOWER(Concat(bookID, ISBN10, ISBN13, author, title, `original-title`, registered)) like LOWER('%$search%')";
                $res = $conn->query($sql);
